...
void solve() {
    ...
    vector<vector<int>> attended(n);
    vector<int> filled(m);

    for (int i = 0; i < m; i++) {
        filled[i] = get<2>(events[i]);
    }
    ...
    for (auto i : best) {
        if (filled[get<2>(i)] > 0) {
            ...
            for (int event : attended[get<1>(i)]) {
                good &= !intersects(event, get<2>(i));
            }

            if (good) {
                attended[get<1>(i)].push_back(get<2>(i));
                filled[get<2>(i)]--;
            }
        }
    }
    ...
    for (int i = 0; i < n; i++) {
        cout << attended[i].size() << ' ';

        for (int j : attended[i]) {
            cout << j + 1 << ' ';
        }
        cout << endl;
    }
